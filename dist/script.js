...
var accordion = function accordion(triggersSelector, itemsSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      if (this.classList.contains('active')) {
        ...
        btn.style.color = '#86b7d2';
      }
    });
  }); //реализация аккордеона без автоматического сворачивания текста при нажатии на следующий триггер
  // btns.forEach(btn => {
  //     btn.addEventListener('click', function() {
  ...
  //     });
  // });
  //реализация аккордеона при помощи css-стилей
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var sum = 0,
      ...
      optionsValue = "0";
  var state;
  Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('assets/db.json').then(function (res) {
    state = res;
  }).catch(function (e) {
...
var drop = function drop() {
  ...
  var fileInputs = document.querySelectorAll('[name="upload"]'); //массив событий, котрые мы будем применять к inputам
...
var mask = function mask(selector) {
  //селектор отмечает импуты, которые нужны для валидации 
      ...
      link.addEventListener('click', function (e) {
        ...
        function step(time) {
          ...
          var progress = time - start,
              r = toBlock < 0 ? Math.max(heightTop - progress / speed, heightTop + toBlock) : Math.min(heightTop + progress / speed, heightTop + toBlock); //насколько будем продвигать анимацию и в какую сторону 
